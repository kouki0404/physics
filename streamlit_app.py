...
if chapter == "1章":
    ...
    if choose == "合成速度":
        ...
        st.session_state.solution_one = st.number_input("(1)回答 ※数字のみ", step=1, format="%.1f")
        st.write("(2) 船が川上に向かって進む場合、川岸で静止している人から見た船の速度は何m/sか。(川下を正とする)")
        st.session_state.solution_two = st.number_input("(2)回答 ※数字のみ", step=1, format="%.1f")
        ...
        if st.button("解答確認"):
            ...
            try:
                ...
                if int(st.session_state.solution_four) == yy / (st.session_state.s - st.session_state.f) * -1 :
                    st.write("(3)②正解")
                else:
                    st.write("(3)②不正解")
